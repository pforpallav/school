...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab8editor) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(check-expect (handle-key (make-es "ab" 1) "\r")
              (make-es "" 0))
(check-expect (handle-key (make-es "ab" 0) "right")
              (make-es "ab" 1))
(check-expect (handle-key (make-es "ab" 1) "left")
              (make-es "ab" 0))
...
(define (handle-key es key)
  (cond [(key=? key "\b")          (backspace es)]
        ...
        ;;  defined left and right functions, similar to the 
        ...
        [(key=? key "left") (left es)]
        [(key=? key "right") (right es)]
...
(check-expect (left  (make-es "ab" 0)) (make-es "ab" 0))
(check-expect (left  (make-es "ab" 1)) (make-es "ab" 0))
(check-expect (right (make-es "ab" 1)) (make-es "ab" 2))
(check-expect (right (make-es "ab" 2)) (make-es "ab" 2))
