...
(define (si-move s)
  (cond [(aim? s) (make-aim (moveufo s)
                            (movetank s))]
        [(fired? s) (make-fired (moveufo s)
                                (movetank s)
                                (movemissile s))]))
;; tank -> tank
;; consumes a tank, produces a tank with x changing according to vel
(check-expect (movetank (make-aim (make-ufo 0 (+ 0 UFO-Y-SPEED) UFO-X-SPEED)
                                  (make-tank 0 TANK-SPEED)))
              (make-tank TANK-SPEED TANK-SPEED))
(check-expect (movetank (make-fired (make-ufo 0 (+ 0 UFO-Y-SPEED) UFO-X-SPEED)
                                    (make-tank 0 TANK-SPEED)
                                    (make-missile 0 MISSILE-SPEED)))
              (make-tank TANK-SPEED TANK-SPEED))

(define (movetank s)
  (cond [(aim? s) (make-tank (+ (tank-loc (aim-tank s))
                                (tank-vel (aim-tank s)))
                             (tank-vel (aim-tank s)))]
        [(fired? s) (make-tank (+ (tank-loc (fired-tank s))
                                  (tank-vel (fired-tank s)))
                               (tank-vel (fired-tank s)))]))

;; ufo -> ufo
;; consumes a tank, produces a ufo with x changing according to UFO-X-SPEED
;(check-expect (moveufo (make-aim (make-ufo 0 (+ 0 UFO-Y-SPEED) UFO-X-SPEED)
;                                 (make-tank 0 TANK-SPEED)))
;              (make-ufo UFO-X-SPEED UFO-Y-SPEED UFO-X-SPEED))
;(check-expect (moveufo (make-aim (make-ufo WIDTH (+ 0 UFO-Y-SPEED) UFO-X-SPEED)
;                                 (make-tank 0 TANK-SPEED)))
;              (make-ufo (- WIDTH UFO-X-SPEED) UFO-Y-SPEED (- UFO-X-SPEED)))


(define (moveufo s)
  (cond [(aim? s)(cond [(>= (ufo-x (aim-ufo s)) WIDTH)
                        (make-ufo (- (ufo-x (aim-ufo s)) (ufo-dx (aim-ufo s)))
                                  (+ (ufo-y (aim-ufo s)) UFO-Y-SPEED)
                                  (- (ufo-dx (aim-ufo s))))]
                       [(<= (ufo-x (aim-ufo s)) 0) 
                        (make-ufo (- (ufo-x (aim-ufo s)) (ufo-dx (aim-ufo s)))
                                  (+ (ufo-y (aim-ufo s)) UFO-Y-SPEED)
                                  (- (ufo-dx (aim-ufo s))))]
                       [else (make-ufo (+ (ufo-x (aim-ufo s)) (ufo-dx (aim-ufo s)))
                                       (+ (ufo-y (aim-ufo s)) UFO-Y-SPEED)
                                       (ufo-dx (aim-ufo s)))])]
        [(fired? s)(cond [(>= (ufo-x (fired-ufo s)) WIDTH)
                          (make-ufo (- (ufo-x (fired-ufo s)) (ufo-dx (fired-ufo s)))
                                    (+ (ufo-y (fired-ufo s)) UFO-Y-SPEED)
                                    (- (ufo-dx (fired-ufo s))))]
                         [(<= (ufo-x (fired-ufo s)) 0) 
                          (make-ufo (- (ufo-x (fired-ufo s)) (ufo-dx (fired-ufo s)))
                                    (+ (ufo-y (fired-ufo s)) UFO-Y-SPEED)
                                    (- (ufo-dx (fired-ufo s))))]
                         [else (make-ufo (+ (ufo-x (fired-ufo s)) (ufo-dx (fired-ufo s)))
                                         (+ (ufo-y (fired-ufo s)) UFO-Y-SPEED)
                                         (ufo-dx (fired-ufo s)))])]))
...
;; missile -> missile
;; consumes a missile, produces a missile with x equal to tank-loc an y being missile speed
(check-expect (movemissile (make-fired (make-ufo 0 (+ 0 UFO-Y-SPEED) UFO-X-SPEED)
                                       (make-tank 0 TANK-SPEED)
                                       (make-missile 0 MISSILE-SPEED)))
              (make-missile 0 MISSILE-SPEED))

(define (movemissile s)
...
(define (si-game-over? s)
  (cond [(aim? s)
         (cond [(>= (ufo-y (aim-ufo s)) HEIGHT) true]
               [else false])]
        [(fired? s)
         (cond [(and (= (missile-x (fired-missile s))
                        (ufo-x (fired-ufo s)))
                     (= (missile-y (fired-missile s))
                        (ufo-y (fired-ufo s))))
                    true]
               [(>= (ufo-y (fired-ufo s)) HEIGHT) true]
               [else false])]))